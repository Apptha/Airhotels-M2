...
class Attributes extends \Magento\Framework\App\Action\Action {
    ...
    public function execute() {
        ...
        if ($currentProductId != "") {
            $product = $this->objectManager->get ( 'Magento\Catalog\Model\Product' )->load ( $currentProductId );
        ...
        $attributeGroupCollection->addFieldToFilter ( 'attribute_group_name', 'Booking Information' );
    ...
    public function textHtml($attributeCode,$isRequired,$attributeName,$product){
        ...
        $html = $html . ' </label><div class="input-box"><input placeholder="'.$attributeName.'" id="' . $attributeCode . '" name="product[' . $attributeCode . ']"  value="';
        /**
        ...
        $html = $html . ' data-validate="{required:true}" ';
         ...
         if ($attributeCode == ('maximum_days'||'minimum_days')) {
        $html = $html . ' data-validate="{checkinteger:true}" ';
        }
        if ($isRequired != 0) {
        ...
        public function booleanHtml($attributeCode,$isRequired,$attributeName,$product){
             ...
             */
            $options = $this->objectManager->create ( 'Magento\Catalog\Model\Product\Attribute\Repository' )->get ( $attributeCode )->getSource()->getAllOptions(false);
